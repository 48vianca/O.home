...
export default function TrpgDetailPage() {
  ...
  const [roll20Css, setRoll20Css] = useState('');
  ...
  // Roll20 원본 CSS를 문자열로 읽어 iframe 문서 안에 직접 주입한다.
  // iframe은 Blob URL이라 부모 페이지의 CSS가 전달되지 않으므로 <link> 대신 <style>로 넣는다.
  useEffect(() => {
    let alive = true;
    Promise.all([
      fetch('/roll20/app.css').then(r => {
        if (!r.ok) throw new Error(`app.css ${r.status}`);
        return r.text();
      }),
      fetch('/roll20/roll20_style.min.css').then(r => {
        if (!r.ok) throw new Error(`roll20_style.min.css ${r.status}`);
        return r.text();
      }),
    ])
      .then(([appCss, rollCss]) => {
        if (alive) setRoll20Css(`${appCss}\n${rollCss}`);
      })
      .catch(err => {
        console.error('[TRPG] Roll20 CSS load failed:', err);
        if (alive) setRoll20Css('');
      });
    return () => { alive = false; };
  }, []);
  ...
  const safeRoll20Css = roll20Css.replace(/<\/style/gi, '<\\/style');
  const inject = `<meta charset="utf-8"><style>${safeRoll20Css}</style><script>
